...
def initialize(os, cluster_type, x_0ring_version)
  ...
  @x_ini_0ring_logsdir = "#{@x_supervision_new_cluster_basedir}/.0x1.ini.0ring/logs"
...
def remote_execute()
  ...
  until @rsync_command_executed
    ...
    if e__service_online?(X_MACHINE_TEMP_IP, X_MACHINE_TEMP_SSH_PORT)
      ...
      puts "#{@x_log_prefix} checking ip/port #{X_MACHINE_TEMP_IP}/#{X_MACHINE_TEMP_SSH_PORT}: UNAVAILABLE"
    end
  end
  ...
  until @x_0ring_ini_ssh_root_phases_command_executed
    if e__service_online?(X_MACHINE_TEMP_IP, X_MACHINE_TEMP_SSH_PORT)
      ...
      abort "ERROR running command:\n#{terminal_0ring_ini_ssh_root_phases_command}" unless system "#{terminal_0ring_ini_ssh_root_phases_command}"
  ...
  remote_check_phase_system_done_0ring_command = "#{ssh_root_temp_command} 'cat /root/#{@x_ini_0ring_dir}/logs/0x1.ini.system.done.0ring 2>/dev/null'"
  until defined? @remote_check_phase_system_done_0ring_checked
    ...
    if e__service_online?(X_MACHINE_TEMP_IP, X_MACHINE_TEMP_SSH_PORT)
      ...
      if remote_check_phase_system_done_0ring.chomp == 'done'
        ...
        puts "#{@x_log_prefix} to follow the main live log, run: \n#{ssh_root_temp_command} tail -f '\~/.0x1.ini.0ring/logs/0x1.ini.root_phase2.0ring'\n\n"
        abort "ERROR running command:\n#{terminal_0ring_ini_ssh_root_phases_command}" unless system "#{terminal_0ring_ini_ssh_root_phases_command}"
...
def remote_checking_system_ready(verbose=true)
  ...
  ssh_mainuser_command = "ssh -p#{X_MACHINE_SSH_PORT} -o PasswordAuthentication=no -o ConnectTimeout=5 #{X_MAINUSER_NAME}@#{X_MACHINE_TEMP_IP}"
